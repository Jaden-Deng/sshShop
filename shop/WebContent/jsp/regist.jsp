...
<script type="text/javascript">
	...
	function check(id, info) {
		...
		//校验确认密码是否正确
		if (id == "repassword"&&uValue != "") {
			 var password = document.getElementById("password").value;
			if(uValue!=password){
				document.getElementById(id + "span").innerHTML = "<font color='red'>"
					+ info + "</font>";
			}else{
				document.getElementById(id + "span").innerHTML = "<font color='green'>密码确认一致</font>";
			}
		}
		if (id == "email") {
			//对电子邮件的验证
			var chackEmail = /^([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+@([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+\.[a-zA-Z]{2,3}$/;
			if (!chackEmail.test(uValue)) {
				document.getElementById(id + "span").innerHTML = "<font color='red'>"
						+ info + "</font>";
				chackEmail.focus();
				return false;
		...
		if (id == "phone") {
			//对电话的验证
			var chackPhone = /^((0\d{2,3}-\d{7,8})|(1[3584]\d{9}))$/;
			if (!chackPhone.test(uValue)) {
				document.getElementById(id + "span").innerHTML = "<font color='red'>"
						+ info + "</font>";
				chackPhone.focus();
				return false;
	 ...
	 //校验用户名
	 var username = document.getElementById("username").value;
	 var userchackmsg = document.getElementById("usernamefont").innerHTML;
	 //校验密码
	 var password = document.getElementById("password").value;
	 //校验确认密码
	 ...
	 if(username==null||username==''||password==null||password==''||repass==null||repass==''||thename==null||thename==''||phone==null||phone==''||addr==null||addr==''){
	 alert("信息未填写完整！");
	 return false;
	 }
	//校验用户名是否重复
	 if(userchackmsg == "用户名已经存在"){
		 alert("用户名已经存在！");
		 return false;
		}
							...
							<tbody>
								<tr>
									...
									<td><input type="text" id="username" name="username"
										class="text" maxlength="20"  required="required" onchange="checkUsername()" onblur="check('username','用户名不能为空！')" ><span id="usernamespan" ><s:fielderror fieldName="username"/></span></td>
								</tr>
								<tr>
									<th><span class="requiredField">*</span>密&nbsp;&nbsp;码:</th>
									<td><input type="password" id="password" name="password"
										class="text" maxlength="20"  required="required" autocomplete="off" onblur="check('password','密码不能为空！')"><span id="passwordspan"  ><s:fielderror fieldName="password"/></span></td>
								</tr>
								<tr>
									<th><span class="requiredField">*</span>确认密码:</th>
									<td><input type="password"  id="repassword" class="text"
										maxlength="20"  required="required" autocomplete="off" onblur="check('repassword','两次输入密码不一致！')"><span id="repasswordspan" ></span></td>
								</tr>
								<tr>
									<th><span class="requiredField">*</span>E-mail:</th>
									<td><input type="text" id="email" name="email"
										class="text" maxlength="200"  required="required" onblur="check('email','请正确填写邮箱！')"><span id="emailspan" ><s:fielderror fieldName="email"/></span></td>
								</tr>
								<tr>
									<th>姓名:</th>
									<td><input type="text" name="name"
										id="name" class="text" maxlength="200"  required="required" onblur="check('name','姓名不能为空！')"><span id="namespan" ></span></td>
								</tr>
								<tr>
									<th>电话:</th>
									<td><input type="text" name="phone" id="phone"
										class="text"  required="required" onblur="check('phone','请正确填写电话！')"><span id="phonespan"></span></td>
								</tr>

								<tr>
									<th>地址:</th>
									<td><input type="text" name="addr" id="addr"
										class="text" maxlength="200"  required="required" onblur="check('addr','请填写您的地址！')"><span id="addrspan"></span></td>
								</tr>
								<tr>
									...
									<td><span class="fieldSet"> <input type="text" id="checkcode" name="checkcode" class="text captcha"
											maxlength="4"  required="required" autocomplete="off" /><img id="checkImage" class="checkImage"
											src="${pageContext.request.contextPath}/checkImg.action" onclick="changeImg()" title="看不清？点击更换验证码"/>
									</span><font color="red"><s:actionerror/></font></td>
								</tr>
								<tr>
									<th>&nbsp;</th>
